...
cmd({
    ...
    try {
        ...
        const { data: commitData } = await axios.get(data.updateUrl);
        ...
        if (latestCommitHash === currentHash) {
            return reply("✅ Your Stark bot is already up-to-date!");
        }

        await reply("🚀 Updating Stark-MD Bot...");

        // Download the latest code
        const zipPath = path.join(__dirname, "latest.zip");
        const { data: zipData } = await axios.get(data.zipUrl, { responseType: "arraybuffer" });
        ...
        const sourcePath = path.join(extractPath, "Stark-MD-main");
